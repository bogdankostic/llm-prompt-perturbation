...
@component
class WorstSenseLexicalVariator(LexicalVariator):
    """
    A component that varies the lexical content of text by replacing words with synonyms from
    incorrect word senses.
    
    This component uses WordNet to find synsets for each word in the text and selects the worst
    synset by recursively removing the best synset according to the word sense disambiguation model.
    ...
    def _process_token(
        ...
        """
        Process a single token and return its varied form if applicable, using the worst synset
        according to the word sense disambiguation model.
