...
@component
class CachedGoogleGenAIChatGenerator(GoogleGenAIChatGenerator):
    """
    A component that extends GoogleGenAIChatGenerator with caching functionality.
    
    This component caches the responses from the GoogleGenAI API and returns cached responses
    when the same messages are provided again. This helps reduce API calls and costs.
    The cache is organized by model name to prevent mixing responses from different models.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize the CachedGoogleGenAIChatGenerator component.
        ...
        self.cache_dir = Path(cache_dir)
        self.cache_dir.mkdir(parents=True, exist_ok=True)
        super(CachedGoogleGenAIChatGenerator, self).__init__(*args, **kwargs)
    ...
    def _get_model_cache_dir(self) -> Path:
        ...
        """
        # Create a safe directory name from the model name
        model_dir = self._model.replace("/", "_")
        model_cache_dir = self.cache_dir / model_dir
        model_cache_dir.mkdir(parents=True, exist_ok=True)
        return model_cache_dir
    ...
    def run(
        ...
        *,
        ...
        # If not in cache, generate response using parent class
        response = super(CachedGoogleGenAIChatGenerator, self).run(
            messages=messages,
            streaming_callback=streaming_callback,
            generation_kwargs=generation_kwargs,
            tools=tools,
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return default_to_dict(
            self,
            cache_dir=str(self.cache_dir),
            **super(CachedGoogleGenAIChatGenerator, self).to_dict()
        )
